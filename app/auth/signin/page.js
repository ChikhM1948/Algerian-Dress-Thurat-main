...
function SignInForm() {
  ...
  const handleCredentialsSignIn = async (e) => {
    ...
    try {
      console.log('SignIn attempt with:', { email: formData.email, password: '***' });
      ...
      if (result?.error) {
        ...
        switch (result.error) {
          ...
          case 'CallbackRouteError':
            setFormError('Authentication service error. Please try again.');
            break;
          default:
        ...
        setDebugInfo('Sign in successful, redirecting to callback URL...');
        router.push(callbackUrl);
      } else {
        setFormError('Unexpected response from authentication service');
        setIsLoading(false);
      }
    } catch (error) {
  ...
  const handleOAuthSignIn = (provider) => {
    setIsLoading(true);
    setDebugInfo(`Attempting OAuth sign in with ${provider}...`);
  ...
  const testConnection = async () => {
    try {
      ...
      setDebugInfo(`DB Test: ${JSON.stringify(result, null, 2)}`);
    } catch (error) {
      setDebugInfo(`DB Test Error: ${error.message}`);
    }
  };
...
function SignInLoading() {
  return (
    <div className="max-w-md mx-auto">
      ...
      <Card className="shadow-xl border-0 overflow-hidden">
        <div className="bg-gradient-to-r from-indigo-700 to-blue-700 px-6 py-4 -mx-5 -mt-5 mb-6">
          <div className="h-6 bg-blue-600 rounded animate-pulse mb-2"></div>
          <div className="h-4 bg-blue-600 rounded animate-pulse w-2/3"></div>
        </div>
